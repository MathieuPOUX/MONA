...
FileSystem::CurrentDirs::CurrentDirs() {
	...
	String::ForEach forEach([this](uint32_t index,const char* value){
...
static int8_t Stat(const char* path, size_t size, Status& status) {
...
uint64_t FileSystem::GetSize(Exception& ex,const char* path, size_t size, uint64_t defaultValue) {
	Status status;
	if (Stat(path, size, status)<=0) {
		ex.set<Ex::System::File>(path, " doesn't exist");
		return defaultValue;
...
bool FileSystem::Delete(Exception& ex, const char* path, size_t size, Mode mode) {
	...
	if (status.st_mode&S_IFDIR) {
		if (!size)
			path = ".";
		if (mode==MODE_HEAVY) {
			FileSystem::ForEach forEach([&ex](const string& path, uint16_t level) {
...
int FileSystem::ListFiles(Exception& ex, const char* path, const ForEach& forEach, Mode mode) {
	...
	do {
		if (wcscmp(fileData.cFileName, L".") != 0 && wcscmp(fileData.cFileName, L"..") != 0) {
			...
			if (iterate && !forEach(file, mode ? (uint16_t(mode) - 1) : 0))
				iterate = false;
		}
...
const char* FileSystem::GetFile(const char* path, size_t& size, size_t& extPos, Type& type, int32_t& parentPos) {
...
string& FileSystem::Resolve(string& path) {
	...
	do {
		...
		if (parentPos <= 0) {
			...
			if (parentPos<0)
				parentPos = 0;
			if (uint32_t(++parentPos) < dirs.size()) {
...
bool FileSystem::Find(string& path) {
	...
	string temp;
	String::ForEach forEach([&path,&temp](uint32_t index,const char* value) {
		temp.assign(value);
#if defined(_WIN32)
			// "path" => path
