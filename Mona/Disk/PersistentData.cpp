...
void PersistentData::processEntry(Exception& ex,Entry& entry) {
	...
	bool hasData=false;
	FileSystem::ForEach forEach([&ex, name, &file, &hasData](const string& path, uint16_t level){
...
bool PersistentData::loadDirectory(Exception& ex, const string& directory, const string& path, const ForEach& forEach) {
	...
	FileSystem::ForEach forEachFile([&ex, this, &hasData, path, &forEach](const string& file, uint16_t level) {
		/// directory
