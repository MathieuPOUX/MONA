...
struct Path : virtual Object {
	...
	uint64_t		size(bool refresh = false) const { return _pImpl ? _pImpl->size(refresh) : 0; }
	int64_t		lastChange(bool refresh = false) const { return _pImpl ? _pImpl->lastChange(refresh) : 0; }
	int64_t		lastAccess(bool refresh = false) const { return _pImpl ? _pImpl->lastAccess(refresh) : 0; }
...
private:
	...
	struct Impl : virtual Object {
		...
		uint64_t	size(bool refresh) const { std::lock_guard<std::mutex> lock(_mutex); return attributes(refresh).size; }
		int64_t	lastChange(bool refresh) const { std::lock_guard<std::mutex> lock(_mutex); return attributes(refresh).lastChange; }
		int64_t	lastAccess(bool refresh) const { std::lock_guard<std::mutex> lock(_mutex); return attributes(refresh).lastAccess; }

		void setAttributes(uint64_t size, int64_t lastAccess, int64_t lastChange);
	private:
		void init();
