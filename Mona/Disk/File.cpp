...
uint64_t File::queueing() const {
	uint64_t queueing(_queueing);
...
bool File::load(Exception& ex) {
	...
	while (_handle != INVALID_HANDLE_VALUE) {
		...
		_path._pImpl->setAttributes(status.st_mode&S_IFDIR ? 0 : (uint64_t)status.st_size, status.st_atime * 1000ll, status.st_mtime * 1000ll);
		_loaded = true;
		return true;
	}
#endif
...
uint64_t File::size(bool refresh) const {
	if (!_loaded)
...
#else
	int64_t current = lseek64(_handle, 0, SEEK_CUR);
	if(current>=0) {
		int64_t size = lseek64(_handle, 0, SEEK_END);
...
void File::reset(uint64_t position) {
	if(!_loaded)
		return;
	_readen = position;
#if defined(_WIN32)
...
int File::read(Exception& ex, void* data, uint32_t size) {
	if (_path.isFolder()) {
...
bool File::write(Exception& ex, const void* data, uint32_t size) {
	if (_path.isFolder()) {
		if (size)
