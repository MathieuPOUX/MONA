...
void IOFile::read(const Shared<File>& pFile, uint32_t size) {
	...
	private:
		...
		bool process(Exception& ex, const Shared<File>& pFile) {
			...
			if (pFile->_pDecoder) {
				...
				private:
					bool process(Exception& ex, const Shared<File>& pFile) {
						uint32_t decoded = pFile->_pDecoder->decode(_pBuffer, _end);
						if (_pBuffer)
...
void IOFile::write(const Shared<File>& pFile, const Packet& packet) {
	...
	private:
		...
		bool process(Exception& ex, const Shared<File>& pFile) {
			uint64_t queueing = (pFile->_queueing -= _packet.size());
...
bool IOFile::run(Exception& ex, const volatile bool& requestStop) {
	...
	while(!requestStop) {
		...
		if (wakeUp.wait((uint32_t)max(1000 - time.elapsed(), 1)))
