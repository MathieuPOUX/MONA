...
struct Logs : virtual Static {
	...
	static void			SetDumpLimit(int32_t limit) { std::lock_guard<std::mutex> lock(_Mutex); _DumpLimit = limit; }
	...
	template <typename ...Args>
	static void Dump(const char* name, const char* data, uint32_t size, Args&&... args) {
		if (!_Dump || _Dumping)
			return;
		_Dumping = true;
		std::lock_guard<std::mutex> lock(_Mutex);
	...
	template <typename ...Args>
	static void Dump(const std::string& name, const char* data, uint32_t size, Args&&... args) { Dump(name.c_str(), data, size, std::forward<Args>(args)...); }

	template <typename ...Args>
	static void DumpRequest(const char* name, const char* data, uint32_t size, Args&&... args) {
		if (_DumpRequest)
			Dump(name, data, size, std::forward<Args>(args)...);
	}
	template <typename ...Args>
	static void DumpRequest(const std::string& name, const char* data, uint32_t size, Args&&... args) { DumpRequest(name.c_str(), data, size, std::forward<Args>(args)...); }

	template <typename ...Args>
	static void DumpResponse(const char* name, const char* data, uint32_t size, Args&&... args) {
		if (_DumpResponse)
			Dump(name, data, size, std::forward<Args>(args)...);
	}
	template <typename ...Args>
	static void DumpResponse(const std::string& name, const char* data, uint32_t size, Args&&... args) { DumpResponse(name.c_str(), data, size, std::forward<Args>(args)...); }

#if defined(_DEBUG)
	// To dump easly during debugging => no name filter = always displaid even if no dump argument
	static void Dump(const char* data, uint32_t size) {
		if (_Dumping)
			return;
		_Dumping = true;
		std::lock_guard<std::mutex> lock(_Mutex);
...
private:
	static void		Dump(const std::string& header, const char* data, uint32_t size);
	...
	static int32_t				_DumpLimit; // -1 means no limit
};
