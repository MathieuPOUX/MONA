...
char*	DiffieHellman::readPublicKey(char* key) const {
	if (!_pDH)
		return NULL;
...
char*	DiffieHellman::readPrivateKey(char* key) const {
	if (!_pDH)
		return NULL;
#if OPENSSL_VERSION_NUMBER < 0x10100000L
...
uint8_t DiffieHellman::computeSecret(Exception& ex, const char* farPubKey, uint32_t farPubKeySize, char* sharedSecret) {
	if (!_pDH && !computeKeys(ex))
		return 0;
	BIGNUM *bnFarPubKey = BN_bin2bn(BIN farPubKey,farPubKeySize,NULL);
	int size = DH_compute_key(BIN sharedSecret, bnFarPubKey, _pDH);
	BN_free(bnFarPubKey);
	if (size <= 0) {
