...
const uint32_t			Thread::MainId(Thread::CurrentId());
...
uint32_t Thread::CurrentId() {
	...
	return (uint32_t)syscall(SYS_thread_selfid);
#elif defined(SYS_gettid)
	return (uint32_t)syscall(SYS_gettid);
#elif defined(__NR_gettid)
	return (uint32_t)syscall(__NR_gettid);
#else
	return (uint32_t)pthread_self(); // in despite of finding the real thread id as displaid in thread list view, use pthread_self...
#endif
}
