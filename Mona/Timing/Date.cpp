...
static const uint16_t _MonthDays[][12] = {
	{ 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 },
	{ 0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335} // leap
};

static int32_t LeapYears(int32_t year) {
	int32_t result(year/4); // every 4 year
...
Date& Date::update(int64_t time,int32_t offset) {
	Time::update(time);
	if (_day == 0) {
		_offset = offset;
		return *this;
	...
	int64_t days((int64_t)floor(time / 86400000.0));
	...
	if (time >= 0)
		++days;
	int32_t year = _year = 0;
	int32_t delta(0);
	while ((year = (int32_t)(days / 366)) != _year) {
		days -= delta;
	...
	_hour = (uint8_t)(time / 3600000);
	time %= 3600000;
	_minute = (uint8_t)(time / 60000);
	time %= 60000;
	_second = (uint8_t)(time / 1000);
	time %= 1000;

	_millisecond = (uint16_t)time;
	return *this;
}
...
int64_t Date::time() const {
	...
	int64_t time = _day - 1 + LeapYears(_year);
	bool isLeap(IsLeapYear(_year));
	if (isLeap && _year > 0)
		--time;
...
void Date::setOffset(int32_t offset) {
	if (_day == 0 || _changed) {
		_offset = offset;
		return;
	}
...
Date& Date::update(int32_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second, uint16_t millisecond) {
	setYear(year);
	setMonth(month);
...
Date& Date::update(int32_t year, uint8_t month, uint8_t day) {
	setYear(year);
	setMonth(month);
	// keep the following line in last position
	setDay(day);
...
void Date::setClock(uint32_t clock) {
	setHour(uint8_t(clock / 3600000));
	clock %= 3600000;
	setMinute(uint8_t(clock / 60000));
	clock %= 60000;
	setSecond(uint8_t(clock / 1000));
	setMillisecond(clock%1000);
}

void Date::setClock(uint8_t hour, uint8_t minute, uint8_t second, uint16_t millisecond) {
	setHour(hour);
	setMinute(minute);
	setSecond(second);
...
void Date::setWeekDay(uint8_t weekDay) {
	weekDay %= 7;
	int8_t delta(weekDay - this->weekDay());
	if (delta==0)
		return;
...
void Date::setYearDay(uint16_t yearDay) {
	if (_day == 0)
		init(); // to get _year
...
void Date::setHour(uint8_t hour) {
	if (_day == 0)
		init();
	if (hour > 59)
		hour = 59;
...
void Date::setMinute(uint8_t minute) {
	if (_day == 0)
		init();
	if (minute>59)
		minute = 59;
...
void Date::setSecond(uint8_t second) {
	if (_day == 0)
		init();
	if (second>59)
		second = 59;
...
void Date::setMillisecond(uint16_t millisecond) {
	if (_day == 0)
		init();
	if (millisecond > 999)
		millisecond = 999;
...
uint8_t Date::weekDay() const {
	if (_day == 0)
		init(); // will assign _weekDay
	else if (_changed || _weekDay==7)
		((Date&)*this).computeWeekDay((int64_t)floor((time()+offset()) / 86400000.0));
	return _weekDay;
}
...
void Date::computeWeekDay(int64_t days) {
	int64_t result(days += 4);
	if (days < 0)
		++result;
	result %= 7;
	if (days<0)
		result += 6;
	_weekDay = (uint8_t)result;
}
...
bool Date::update(Exception& ex, const char* current, size_t size, const char* format) {
	...
	int32_t year(0), offset(Timezone::LOCAL);
