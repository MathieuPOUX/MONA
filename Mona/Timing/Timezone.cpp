...
static const std::map<const char*, pair<int32_t, bool>, String::IComparator> _Timezones({
...
int32_t Timezone::ParseTZOffset(string::const_iterator& it,const string::const_iterator& end,int32_t defaultValue) {
	if (it == end)
		return defaultValue;
	bool ahead(false);
	if (*it == '-') { ahead = true; ++it; }
...
bool Timezone::ParseTZRule(string::const_iterator& it,const string::const_iterator& end,TransitionRule& rule) {
	if (it == end)
		return false;
	TransitionRule::Type type(TransitionRule::TYPE_NONE);
	uint8_t month(0), week(0), day(0);
	if (*it == 'J' && ++it!=end) {
	...
	if (type==TransitionRule::TYPE_NONE)
		return false;
...
bool Timezone::readTZDatabase(const string& path) {
	...
	BinaryReader reader(buffer.data(), buffer.size(), Bytes::ORDER_BIG_ENDIAN);
	...
	if (readTZData<int32_t>(reader) && version >= '2' && reader.available()>4 && memcmp(reader.current(), "TZif", 4) == 0) {
		reader.next(5); // skip TZif2
		readTZData<int64_t>(reader);
	}
	...
	if (reader.available() < 2)
		return false;
	*(STR reader.current() + reader.available() - 1) = '\0'; // to be sure that there is a null-terminated character before the end
...
void Timezone::fillDefaultTransitionRule(const SYSTEMTIME& date,bool isDST,Timezone::TransitionRule& rule) {
	if (date.wYear > 0) {
		...
		_transitions[Date((int32_t)date.wYear, (uint8_t)date.wMonth, (uint8_t)date.wDay, (uint8_t)date.wHour, (uint8_t)date.wMinute, (uint8_t)date.wSecond,0,Timezone::GMT)].set(isDST ? _dstOffset : _offset,isDST);
		return;
	}
		
	// absolute
	rule.month = (uint8_t)date.wMonth;
	rule.week = (uint8_t)date.wDay;
	rule.day = (uint16_t)date.wDayOfWeek;
...
Timezone::Timezone() : _offset(0),_dstOffset(3600000) {
	...
	_offset=(-(int32_t)timezoneInfo.Bias*60000);
	if (result != TIME_ZONE_ID_UNKNOWN)
		_dstOffset = (-(int32_t)timezoneInfo.DaylightBias*60000);
	_dstOffset += _offset;
   ...
   if (_name.empty())
	...
	for (int32_t year = 1916; year < (date.year() + 20); ++year) {
...
int32_t Timezone::offset(const Date& date, bool& isDST) {
	isDST = false;
...
int64_t Timezone::ruleToTime(int32_t year, bool isDST, const TransitionRule& rule) {
	
	Date date(year,rule.month,1,Timezone::GMT);
	date.setClock(rule.clock);

	uint16_t day(rule.day);
	switch (rule.type) {
...
int32_t Timezone::Offset(const char* code ,bool& isDST) {
