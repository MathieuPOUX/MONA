...
struct Stopwatch : virtual Object {
	...
	int64_t elapsed() const { return _running ? std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - _start).count() : _elapsed; }
	operator int64_t() const { return this->elapsed(); }
