...
#include "Mona/Timing/Stopwatch.h"
...
private:
	...
	// POOLTESTS
	void runAll(uint32_t loop);
		/// \brief Run all tests

	void runOne(const std::string& mod, uint32_t loop);
		/// \brief Run the test with 'mod' name

	void runAt(const std::string& mod, uint32_t loop);
