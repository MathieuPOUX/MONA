...
struct Net : virtual Object {
	...
	static uint32_t GetRecvBufferSize() { return _Net._recvBufferSize; }
	static void   SetRecvBufferSize(uint32_t size) { _Net._recvBufferSize = size; }
	static void	  ResetRecvBufferSize() { _Net._recvBufferSize = _Net._recvBufferDefaultSize; }
	static uint32_t GetSendBufferSize() { return _Net._sendBufferSize; }
	static void	  SetSendBufferSize(uint32_t size) { _Net._sendBufferSize = size; }
	static void	  ResetSendBufferSize() { _Net._sendBufferSize = _Net._sendBufferDefaultSize; }

	static uint32_t GetInterfaceIndex(const SocketAddress& address);

	static uint16_t ResolvePort(Exception& ex, const char* service);
	static uint16_t ResolvePort(Exception& ex, const std::string& service) { return ResolvePort(ex, service.c_str()); }
...
private:
	...
	std::atomic<uint32_t> _recvBufferSize;
	std::atomic<uint32_t> _sendBufferSize;
