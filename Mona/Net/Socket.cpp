...
bool Socket::setRecvBufferSize(Exception& ex, uint32_t size) {
	if (!setOption(ex, SOL_SOCKET, SO_RCVBUF, size))
		return false;
	_recvBufferSize = size;
	return true;
}
bool Socket::setSendBufferSize(Exception& ex, uint32_t size) {
...
bool Socket::joinGroup(Exception& ex, const IPAddress& ip, uint32_t interfaceIndex) {
...
void Socket::leaveGroup(const IPAddress& ip, uint32_t interfaceIndex) {
	Exception ex;
	if (ip.family() == IPAddress::IPv4) {
		struct ip_mreq mreq;
		memcpy(&mreq.imr_multiaddr, ip.data(), ip.size());
...
bool Socket::connect(Exception& ex, const SocketAddress& address, uint16_t timeout) {
	if (_ex) {
		ex = _ex;
		return false;
	}
...
int Socket::receive(Exception& ex, char* buffer, uint32_t size, int flags, SocketAddress* pAddress) {
	if (_ex) {
		ex = _ex;
		return -1;
	}
	...
	do {
		if (pAddress) {
			...
			NET_SOCKLEN addrSize = sizeof(addr);
			if ((rc = ::recvfrom(_id, buffer, size, flags, reinterpret_cast<sockaddr*>(&addr), &addrSize)) >= 0)
...
int Socket::sendTo(Exception& ex, const char* data, uint32_t size, const SocketAddress& address, int flags) {
	if (_ex) {
		ex = _ex;
		return -1;
	}
	...
	do {
		if (type == TYPE_DATAGRAM && address) // for TCP socket, address must be null!
			rc = ::sendto(_id, data, size, flags, address.data(), address.size());
		else
			rc = ::send(_id, data, size, flags);
	} while (rc < 0 && (error=Net::LastError()) == NET_EINTR);
	if (rc < 0) {
	...
	if (uint32_t(rc) < size && type == TYPE_DATAGRAM) {
...
int Socket::write(Exception& ex, const Packet& packet, const SocketAddress& address, int flags) {
	...
	} else if (uint32_t(sent) >= packet.size()) {
		_sending = false;
		return packet.size();
	}
