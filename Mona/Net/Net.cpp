...
Net::Stats& Net::Stats::Null() {
	static struct Null : Stats, virtual Object {
		...
		uint64_t	recvByteRate() const { return 0; }
		Time	sendTime() const { return 0; }
		uint64_t	sendByteRate() const { return 0; }
		uint64_t	queueing() const { return 0; }
	} Null;
	return Null;
}
...
uint16_t Net::ResolvePort(Exception& ex, const char* service) {
...
uint32_t Net::GetInterfaceIndex(const SocketAddress& address) {
	if (!address.host())
		return 0;
#if defined(_WIN32)
	DWORD index = 0;
