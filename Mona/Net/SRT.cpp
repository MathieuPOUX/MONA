...
uint32_t SRT::Socket::available() const {
	if(!_shutdownRecv) {
		int32_t events;
		Exception ignore;
...
bool SRT::Socket::connect(Exception& ex, const SocketAddress& address, uint16_t timeout) {
	if (_ex) {
		ex = _ex;
		return false;
	}
...
int SRT::Socket::receive(Exception& ex, char* buffer, uint32_t size, int flags, SocketAddress* pAddress) {
	if (_ex) {
		ex = _ex;
		return -1;
	}
	...
	// Read the message
	int result = ::srt_recvmsg(_id, buffer, size); // /!\ SRT expect at least a buffer of 1316 bytes
	if (result == SRT_ERROR) {
...
int SRT::Socket::sendTo(Exception& ex, const char* data, uint32_t size, const SocketAddress& address, int flags) {
	if (_ex) {
		ex = _ex;
		return -1;
	}
	...
	for (size_t i = 0; i < size; i += chunk) {
		...
		if (::srt_send(_id, data + i, chunk) < 0) {
			ex.set<Ex::Net::Socket>("SRT: send error; ", " ", ::srt_getlasterror_str());
			return -1;
		}
	}*/
	int sent = ::srt_send(_id, data, size);
	if (sent <= 0) {
		SetException(ex);
		return -1;
	}
...
bool SRT::Socket::setRecvBufferSize(Exception& ex, uint32_t size) {
	if (!setOption(ex, ::SRTO_UDP_RCVBUF, size))
		return false;
	_recvBufferSize = size;
	return true;
}
bool SRT::Socket::setSendBufferSize(Exception& ex, uint32_t size) {
...
bool SRT::Socket::setPassphrase(Exception& ex, const char* data, uint32_t size) {
	if (_ex) {
		ex = _ex;
		return false;
	}
...
bool SRT::Socket::setStreamId(Exception& ex, const char* data, uint32_t size) {
	if (_ex) {
		ex = _ex;
		return false;
	}
