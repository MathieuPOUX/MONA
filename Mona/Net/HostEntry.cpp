	...
	for (; ainfo; ainfo = ainfo->ai_next) {
		...
		if (ainfo->ai_addrlen && ainfo->ai_addr) {
			if (ainfo->ai_addr->sa_family == AF_INET6)
				_addresses.emplace(reinterpret_cast<sockaddr_in6*>(ainfo->ai_addr)->sin6_addr, (uint32_t)reinterpret_cast<struct sockaddr_in6*>(ainfo->ai_addr)->sin6_scope_id);
