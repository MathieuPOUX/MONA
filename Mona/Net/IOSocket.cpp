...
#if defined(SRT_API)
	#include "Mona/Net/IOSRTSocket.h"
#endif
...
void IOSocket::read(const Shared<Socket>& pSocket, int error) {
		...
		private:
			...
			private:
				void handle(const Shared<Socket>& pSocket) {
					...
					uint32_t receiving = --pSocket->_receiving;
					if (!_pThread)
						return;
	...
	private:
		...
		private:
			void handle(const Shared<Socket>& pSocket) {
				uint32_t receiving = _pBuffer->size();
...
#define MAXEVENTS  1024
	...
	while ((result=GetMessage(&msg, _system, 0, 0)) > 0) {
		...
		uint32_t event(WSAGETSELECTEVENT(msg.lParam));
		// Map to display event string when need
		/*static map<uint32_t, const char*> events({
			{ FD_READ, "READ" },
			{ FD_WRITE, "WRITE" },
			{ FD_ACCEPT, "ACCEPT" },
			{ FD_CLOSE, "CLOSE" },
