...
struct SocketAddress : private IPAddress, virtual Object {
	...
	SocketAddress&  setPort(uint16_t port) { IPAddress::setPort(port); return self; }
	...
	static uint16_t SplitLiteral(const char* value, std::string& host);
	static uint16_t SplitLiteral(const std::string& value, std::string& host) { return SplitLiteral(value.data(),host); }
...
private:
	...
	bool setIntern(Exception& ex, const char* host, uint16_t port, bool resolveHost) { return resolveHost ? IPAddress::setWithDNS(ex, host, port) : IPAddress::set(ex, host, port); }
