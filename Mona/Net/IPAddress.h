...
struct IPAddress : virtual Object {
	...
	IPAddress(const in6_addr& addr, uint32_t scope = 0);
	IPAddress& set(const in_addr& addr) { return set(addr, port()); }
	IPAddress& set(const in6_addr& addr, uint32_t scope = 0) { return set(addr, scope, port()); }
...
protected:
	...
	IPAddress(const IPAddress& other, uint16_t port);

	IPAddress& set(const in_addr& addr, uint16_t port);
	IPAddress& set(const in6_addr& addr, uint32_t scope, uint16_t port);

	IPAddress& set(const sockaddr& addr);
	IPAddress& set(const IPAddress& other, uint16_t port);

	bool set(Exception& ex, const char* address, uint16_t port);
	bool set(Exception& ex, const char* address, Family family, uint16_t port);
	bool setWithDNS(Exception& ex, const char* address, uint16_t port) { return setInternWithDNS(ex,address,port); }
